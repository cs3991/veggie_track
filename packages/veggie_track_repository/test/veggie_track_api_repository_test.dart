...
void main() {
  group('Repository', () {
    ...
    test('Adding food type', () async {
      ...
      expect(repo.addFoodType(FoodType(label: 'apple', carbonFootprint: 0.5)),
    ...
    test('Adding day', () async {
      ...
      await repo.addFoodType(FoodType(label: 'vegetable', carbonFootprint: 1));
      await repo.addFoodType(FoodType(label: 'eggs', carbonFootprint: 0.23));
      await repo
          .addFoodType(FoodType(label: 'chicken', carbonFootprint: 0.589));
      await repo.addFoodType(FoodType(label: 'carbs', carbonFootprint: 1.25));

      var date = DateTime(2023, DateTime.january, 22);
      await repo.addDay(Day(date: date, lunch: [
        Food(
            foodType: await repo.getFoodTypeByLabel('vegetable'),
            quantity: 200),
    ...
    test('Relationships between day and foodtype', () async {
      ...
      var date = DateTime(2023, DateTime.january, 22);
      await repo.addFoodType(FoodType(label: 'apple', carbonFootprint: 0.5));
      await repo.addFoodType(FoodType(label: 'banana', carbonFootprint: 0.5));
      await repo.addDay(Day(date: date, lunch: [
